 ...
 class pPie
  {
   ...
   function draw2DPie($X,$Y,$Format="")
    {
     ...
     // crmv@30014
     $DrawLabelLine	= isset($Format["DrawLabelLine"]) ? $Format["DrawLabelLine"] : TRUE;
     $DrawLabelArrow= isset($Format["DrawLabelArrow"]) ? $Format["DrawLabelArrow"] : TRUE;
     $LabelDistance	= isset($Format["LabelDistance"]) ? $Format["LabelDistance"] : 25;
     $LabelArrowSize= isset($Format["LabelArrowSize"]) ? $Format["LabelArrowSize"] : 8;
     ...
     foreach($Values as $Key => $Value)
      {
       ...
       if ( $RecordImageMap && !$Shadow ) { $this->pChartObject->addToImageMap("POLY",$this->arraySerialize($Plots),$this->pChartObject->toHTMLColor($Palette[$ID]["R"],$Palette[$ID]["G"],$Palette[$ID]["B"]),$Data["Series"][$Data["Abscissa"]]["Data"][$Key],$Value, FALSE, round(( 100 / $SerieSum ) * $Value,$Precision) ); } // crmv@30014

       if ( $DrawLabels && !$Shadow && !$SecondPass )
        {
         if ( $LabelColor == PIE_LABEL_COLOR_AUTO )
          { $Settings = array("FillR"=>$Palette[$ID]["R"],"FillG"=>$Palette[$ID]["G"],"FillB"=>$Palette[$ID]["B"],"Alpha"=>$Palette[$ID]["Alpha"],'DrawLabelLine'=>$DrawLabelLine,'LabelDistance'=>$LabelDistance, 'LabelArrowSize'=>$LabelArrowSize, 'DrawArrow'=>$DrawLabelArrow);} // crmv@30014
         else
          { $Settings = array("FillR"=>$LabelR,"FillG"=>$LabelG,"FillB"=>$LabelB,"Alpha"=>$LabelAlpha, 'DrawLabelLine'=>$DrawLabelLine, 'LabelDistance'=>$LabelDistance, 'LabelArrowSize'=>$LabelArrowSize, 'DrawArrow'=>$DrawLabelArrow); } // crmv@30014
     ...
     foreach($Slices as $SliceID => $Plots)
      {
       ...
       if ( $RecordImageMap && !$Shadow ) { $this->pChartObject->addToImageMap("POLY",$this->arraySerialize($Top),$this->pChartObject->toHTMLColor($Settings["R"],$Settings["G"],$Settings["B"]),$Data["Series"][$Data["Abscissa"]]["Data"][count($Slices)-$SliceID-1],$Values[$SliceID], FALSE, round(( 100 / $SerieSum ) * $Values[$SliceID],$Precision) ); } // crmv@30014
      }
     ...
     foreach($Data["Series"][$Data["Abscissa"]]["Data"] as $Key => $Value)
      {
       ...
       $Boxes[$Key] = $BoxArray;
       $YStep = max(abs($BoxArray[0]['Y'] - $BoxArray[3]['Y']), $BoxSize) + 5;
     ...
     foreach($Data["Series"][$Data["Abscissa"]]["Data"] as $Key => $Value)
      {
       ...
       //$this->pChartObject->drawFilledRectangle($X+1,$Y+1,$X+$BoxSize+1,$Y+$BoxSize+1,array("R"=>0,"G"=>0,"B"=>0,"Alpha"=>20)); // crmv@30014 - remove shadow in legend boxes
     ...
     if ( !$Stacked )
      {
       ...
       $Settings["Length"] = isset($Settings["LabelDistance"]) ? $Settings["LabelDistance"] : 25; // crmv@30014
       $Settings["Size"]   = isset($Settings["LabelArrowSize"]) ? $Settings["LabelArrowSize"] : 8; // crmv@30014
       $Settings["DrawLine"] = isset($Settings["DrawLabelLine"]) ? $Settings["DrawLabelLine"] : TRUE; // crmv@30014
       $Settings["DrawArrow"] = isset($Settings["DrawArrow"]) ? $Settings["DrawArrow"] : TRUE; // crmv@30014
   ...
   function draw2DRing($X,$Y,$Format="")
    {
     $OuterRadius	= isset($Format["OuterRadius"]) ? $Format["OuterRadius"] : 60; // crmv@30014
     $Precision		= isset($Format["Precision"]) ? $Format["Precision"] : 0;
     $InnerRadius	= isset($Format["InnerRadius"]) ? $Format["InnerRadius"] : 30; // crmv@30014
     $Border		= isset($Format["Border"]) ? $Format["Border"] : FALSE;
     $BorderR		= isset($Format["BorderR"]) ? $Format["BorderR"] : 255;
     $BorderG		= isset($Format["BorderG"]) ? $Format["BorderG"] : 255;
     $BorderB		= isset($Format["BorderB"]) ? $Format["BorderB"] : 255;
   ...
   function draw3DRing($X,$Y,$Format="")
    {
       ...
       foreach($Values as $Key => $Value)
        {
         ...
         elseif ( $WriteValues == PIE_VALUE_NATURAL )
          ...
          $Label = "";*/
         $Label = $Data["Series"][$Data["Abscissa"]]["Data"][count($Values)-$Key-1];
         // crmv@30014e
     ...
     // crmv@30014 - added to draw values
     if ( $WriteValues && !$Shadow )
     {
     	...
     	foreach($Values as $Key => $Value)
     	{
     		...
     		{
     			$Xc = cos(($Angle-90)*PI/180) * (($OuterRadius-$InnerRadius)/2+$InnerRadius) + $X;
     			$Yc = sin(($Angle-90)*PI/180) * (($OuterRadius-$InnerRadius)/2+$InnerRadius)*$SkewFactor + $Y;
     			$Align = TEXT_ALIGN_MIDDLEMIDDLE;
     		}
     		...
     		$this->pChartObject->drawText($Xc,$Yc-$SliceHeight,$Display,array("Align"=>$Align,"R"=>$ValueR,"G"=>$ValueG,"B"=>$ValueB));
     		$Offset = $EndAngle;
