...
class RecurringType {
	...
	function __construct($repeat_arr) {
		...
		$this->rpt_count = $repeat_arr['count']; // crmv@185576
	...
	public static function fromUserRequest($requestArray) {
		...
		if ($requestArray['type'] == 'Weekly') {
			...
			if ($requestArray['repeatmonth_type'] == 'date') {
				...
				// crmv@185501 - removed useless code!
			}
		}
	...
	// crmv@185576
	/**
	 * Create the object from a ical string
	 * If unsupported values are passed, null is returned
	 */
	public static function fromRRule($rrule, $activity = array()) {
		global $default_timezone;
		
		// map RFC 5545 frequencies to vte freqs
		$freqMap = array('DAILY' => 'Daily', 'WEEKLY' => 'Weekly', 'MONTHLY' => 'Monthly', 'YEARLY' => 'Yearly');
		$dayMap = array("SU" => 0, "MO" => 1, "TU" => 2, "WE" => 3, "TH" => 4, "FR" => 5, "SA" => 6);
		$seqMap = array(-1 => 'last', 1 => 'first', 2 => 'second', 3 => 'third', 4 => 'fourth', 5 => 'fifth');
		
		// check for unsupported properties
		if ($rrule['BYYEARDAY']) return null;
		if ($rrule['BYMONTH']) return null;
		if ($rrule['BYWEEKNO']) return null;
		if ($rrule['BYHOUR']) return null;
		if ($rrule['BYMINUTE']) return null;
		if ($rrule['BYSECOND']) return null;
		if ($rrule['BYSETPOS']) return null;
		
		//if ($rrule['WKST']) return null; // this is ignored
		
		$repeatInfo = array();
		
		$repeatInfo['type'] = $freqMap[$rrule['FREQ']];
		if (empty($repeatInfo['type'])) return null;
		
		$repeatInfo['repeat_frequency'] = $rrule['INTERVAL'] ?: 1;
		
		$repeatInfo['startdate'] = $activity['date_start'] ?: date('Y-m-d');
		$repeatInfo['starttime'] = $activity['time_start'] ?: date('H:i');
		
		if ($rrule['UNTIL']) {
			$dt = self::icalTS2Date($rrule['UNTIL']);
			$repeatInfo['enddate'] = $dt->format('Y-m-d');
			$repeatInfo['endtime'] = $dt->format('H:i');
		} elseif ($rrule['COUNT']) {
			$repeatInfo['count'] = $rrule['COUNT'];
		} else {
			$repeatInfo['enddate'] = $activity['due_date'] ?: date('Y-m-d');
			$repeatInfo['endtime'] = $activity['time_end'];
		}
		
		if ($repeatInfo['type'] == 'Weekly') {
			$repeatInfo['dayofweek_to_repeat'] = array();
			if ($rrule['BYDAY']) {
				// crmv@186753
				if (count($rrule['BYDAY']) == 1) {
					$dayn = $dayMap[$rrule['BYDAY']['DAY']];
					$repeatInfo['dayofweek_to_repeat'][] = $dayn;
				} else {
				...
				// use the current date as day
				$startTs = strtotime($repeatInfo['startdate'].' '.$repeatInfo['starttime'].':00');
				$startDow = date('w', $startTs);
				$repeatInfo['dayofweek_to_repeat'][] = $startDow;
			}
			
		} elseif ($repeatInfo['type'] == 'Monthly') {
			if ($rrule['BYMONTHDAY']) {
				$repeatInfo['repeatmonth_type'] = 'date';
				$repeatInfo['repeatmonth_date'] = $rrule['BYMONTHDAY'];
			} else {
				
				if ($rrule['BYDAY']) {
					$repeatInfo['dayofweek_to_repeat'] = array();
					$repeatInfo['repeatmonth_type'] = 'day';
					/*foreach ($rrule['BYDAY'] as $day) {
						$seq = $seqMap[]
					}*/
					$seq = $seqMap[$rrule['BYDAY'][0]];
					$dayn = $dayMap[$rrule['BYDAY']['DAY']];
					
					$repeatInfo['repeatmonth_daytype'] = $seq;
					$repeatInfo['dayofweek_to_repeat'][] = $dayn;

				} else {
					// use the current date as day
					$startTs = strtotime($repeatInfo['startdate'].' '.$repeatInfo['starttime'].':00');
					$startDom = date('d', $startTs);
					$repeatInfo['repeatmonth_type'] = 'date';
					$repeatInfo['repeatmonth_date'] = $startDom;
				}
			}
		}
	...
	protected static function icalTS2Date($datetime){
		global $default_timezone;
		...
		$dt->setDate($datetime['year'], $datetime['month'], $datetime['day']);
		$dt->setTime(intval($datetime['hour']), intval($datetime['min']), intval($datetime['sec']));
	...
	function getUserRecurringInfo() {
		...
		if ($recurringType == 'Weekly') {
			...
			if ($this->repeat_monthby == 'date') {
				...
				$recurringInfo['dayofweek_to_repeat'][0] = $dayOfWeek; // crmv@185501
				// crmv@185501 - removed useless code
			}
		}
		return $recurringInfo;
	...
	function getDisplayRecurringInfo() {
		...
		$displayRecurringData['recurringcheck'] = getTranslatedString('LBL_YES', 'Calendar');
		$displayRecurringData['repeat_frequency'] = $this->getRecurringFrequency();
		$displayRecurringData['recurringtype'] = $this->getRecurringType();
		$edate = $this->getRecurringEndDate();
		if ($edate) {
			$displayRecurringData['end_date'] = $edate->get_DB_formatted_date();
			$displayRecurringData['end_date_user'] = getDisplayDate($displayRecurringData['end_date']);
		}
		$displayRecurringData['count'] = $this->getRecurringCount();

		if ($this->getRecurringType() == 'Weekly') {
			$noOfDays = count($recurringInfo['dayofweek_to_repeat']);
			$translatedRepeatDays = array();
			for ($i = 0; $i < $noOfDays; ++$i) {
				$translatedRepeatDays[] = getTranslatedString('LBL_DAY' . $recurringInfo['dayofweek_to_repeat'][$i], 'Calendar');
			}
			...
			if ($recurringInfo['repeatmonth_type'] == 'date') {
				...
				$displayRecurringData['repeat_str'] = getTranslatedString('on', 'Calendar')
						. ' ' . $recurringInfo['repeatmonth_date']
						. ' ' . getTranslatedString('day of the month', 'Calendar');
			} else {
				...
				$translatedRepeatDay = getTranslatedString('LBL_DAY' . $recurringInfo['dayofweek_to_repeat'][0], 'Calendar');

				$displayRecurringData['repeat_str'] = getTranslatedString('on', 'Calendar')
						. ' ' . getTranslatedString($recurringInfo['repeatmonth_daytype'], 'Calendar')
						. ' ' . $translatedRepeatDay;
			}
		}
			
		if ($displayRecurringData['count']) {
			$displayRecurringData['repeat_str'] .= " ".str_replace("%s", $displayRecurringData['count'], strtolower(getTranslatedString('LBL_FORXTIMES', 'Calendar')));
		} elseif ($displayRecurringData['end_date']) {
			$displayRecurringData['repeat_str'] .= " ".strtolower(getTranslatedString('LBL_UNTIL', 'Calendar'))." ".$displayRecurringData['end_date_user'];
		}
		
		$displayRecurringData['full_text'] = getTranslatedString('LBL_REPEATEVENT', 'Calendar')
			." ".$displayRecurringData['repeat_frequency'].' '.strtolower(getTranslatedString($displayRecurringData['recurringtype'], 'Calendar'))
	...
	function _getRecurringDates() {
		...
		$count = $this->rpt_count; // crmv@185576
		$keepGoing = ($count > 0); // crmv@185576
		
		while ($keepGoing || $tempdate <= $enddate) { // crmv@185576
			...
			if ($this->recur_type == 'Daily') {
				...
				if($keepGoing || $tempdate <= $enddate) { // crmv@185576
				...
				for ($i = 0; $i < count($this->dayofweek_to_rpt); $i++) {
					...
					if ($repeatDate > $startdate && ($keepGoing || $repeatDate <= $enddate)) { // crmv@185576
				...
				if ($this->repeat_monthby == 'date') {
					...
					// crmv@185501
					$ns = array('first' => 1, 'second' => 2, 'third' => 3, 'fourth' => 4);
					if (array_key_exists($this->rptmonth_daytype, $ns)) {
						$n = $ns[$this->rptmonth_daytype];
						$date_arr = Array(
							'day' => 1,
							'month' => $month,
							'year' => $year
						...
						$tempdateObj = new vt_DateTime($date_arr, true);
						$firstdayofmonthObj = $this->getNthdayofmonthObj($this->dayofweek_to_rpt[0], $n, $month, $year);
						if ($firstdayofmonthObj->get_DB_formatted_date() <= $tempdate) {
							...
							$tempdateObj = $this->getNthdayofmonthObj($this->dayofweek_to_rpt[0], $n, $month, $firstdayofmonthObj->getYear());
						} else {
			...
			// crmv@185576
			if ($keepGoing) {
				$keepGoing = count($recurringDates) <= $count;
			}
			// crmv@185576e
		}
	...
	// crmv@185501
	/**
	 * Get the nth day of week of the specified month
	 * @param int $dayofweek The day of the week (1-7)
	 * @param int $n The nth day (1-4)
	 * @param int $month The month (1-12)
	 * @param int $year The year
	 *
	 * @return string/null The date or null if the day doesn't exist (ex: 5th day)
	 */
	function getNthdayofmonth($dayofweek, $n, $month, $year) {
		$month = sprintf("%02d", intval($month));
		
		$next ="$year-$month-".sprintf("%02d", (8 - intval($dayofweek))).' 12:00:00'; // limit tz/dst problems
		$nextTs = strtotime($next);
		$nextDow = date('N', $nextTs);
		
		$nthDay = $n * 7 - $nextDow + 1;
		
		// check if out of range
		if ($nthDay > 28) {
			$totDays = date('t', $nextTs);
			if ($nthDay > $totDays) return null;
		}
		
		return "$year-$month-".sprintf("%02d", intval($nthDay));
	}
	
	/**
	 * Same as getNthdayofmonth, but return a vt_DateTime object instead
	 */
	function getNthdayofmonthObj($dayofweek, $n, $month, $year) {
		$date = $this->getNthdayofmonth($this->dayofweek_to_rpt[0], $n, $month, $year);
