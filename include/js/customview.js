...
//crmv@96450 crmv@101683 crmv@113771 crmv@115268 crmv@185753
var fieldValueArr=new Array('Text','TextArea','Number','Percent','Currency','Date','Email','Phone','URL','Checkbox','Picklist','MultiSelectCombo','Picklistmulti','Skype','User','AllUser','CustomUser','Group','Button','Table','AttachDocuments','Hour');
var fieldTypeArr=new Array('text','textarea','number','percent','currency','date','email','phone','url','checkbox','picklist','multiselectcombo','picklistmulti','skype','user','alluser','customuser','group','button','table','attachdocuments','hour');
//crmv@96450e crmv@101683e crmv@113771e crmv@115268e crmv@185753e
/* crmv@174986
fieldValueArr.push('Role');
fieldTypeArr.push("role");
crmv@174986e */
	...
	if(combo_val != '')
	{
		if(leadtype == type)
		{
			...
			else
			{
				...
				switch (lead_tod[0]) {
					case "V"  :
						if(lead_tod[3] <= tod[3] || ((typeof lead_tod[3] != 'undefined') && (typeof tod[3] == 'undefined'))) //crmv@99019
							return true;
						//crmv@49738
						else if ((typeof lead_tod[3] == 'undefined') && (typeof tod[3] == 'undefined')) {
							return true;
						}
						//crmv@49738e
						else
...
//vtc
function selFieldTypeLayoutEditor(id,scrollLayer,bool,blockid) {
    currFieldIdx = id;
    var type = fieldTypeArr[id];
    //crmv@101683 crmv@113771
	var lengthLayer = jQuery('#lengthdetails_'+blockid);
	var decimalLayer = jQuery('#decimaldetails_'+blockid);
	var pickListLayer = jQuery('#picklistdetails_'+blockid);
	var userListLayer = jQuery('#usersdetails_'+blockid);
	var onClickLayer = jQuery('#onclickdetails_'+blockid);
	var codeLayer = jQuery('#codedetails_'+blockid);
	lengthLayer.hide();
	decimalLayer.hide();
	pickListLayer.hide();
	userListLayer.hide();
    ...
    if (type=='text') {
		...
		pickListLayer.show();
	} else if (type=='customuser') {
		userListLayer.show();
		jQuery("#fldCustomUserPick_"+blockid+' option').removeProp('selected');
    } else if (type=='button') {
		onClickLayer.show();
		codeLayer.show();
	}
...
function validateLayoutEditor(blockid) {
	var nummaxlength = 255;
    var fieldtype = document.getElementById('selectedfieldtype_'+blockid).value;
	var mode = document.getElementById('cfedit_mode').value;
    if(fieldtype == "" && mode != 'edit')
	...
	if (!emptyCheck("fldLabel_"+blockid,"Label","text"))
            return false
     var re2=/[&\<\>\:\'\"\,\_\#]/; // crmv@194270
    if (re2.test(str))
    {
		alert(alert_arr.SPECIAL_CHARACTERS+" & < > ' \" : , _ # "+alert_arr.NOT_ALLOWED) // crmv@194270
		return false;
    }
	if (lengthLayer.is(":visible")) {
		if (!emptyCheck('fldLength_'+blockid,"Length"))
				return false
		if (!intValidate('fldLength_'+blockid,"Length"))
				return false
	...
	var decimallength = '';
	if (decimalLayer.is(":visible") && document.getElementById('fldDecimal_'+blockid) != null)
      	decimallength = document.getElementById("fldDecimal_"+blockid).value;
    ...
    if (lengthLayer.is(":visible") && !numConstComp('fldLength_'+blockid,"Length","LE",nummaxlength)) //crmv@122176
            return false
	var picklistObj=document.getElementById("fldPickList_"+blockid);
    if (pickListLayer.is(":visible")) {
		if (emptyCheck("fldPickList_"+blockid,"Picklist values")) {
			var pickListAry=new Array();
			pickListAry=splitValues(picklistObj); // crmv@109537
