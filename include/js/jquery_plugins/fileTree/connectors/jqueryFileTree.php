...
// crmv@65455 - rewritten as a class, addedd basic configuration

$ftc = new FileTreeConnector();
$ftc->process($_REQUEST);



class FileTreeConnector {

	public $config = array(
		'root' => '../../../../../dataimport/',
		'show_hidden' => false,
		'show_subdirs' => true,
		'allow_types' => array('csv', 'txt'),
		'fs_encoding' => 'UTF-8',
	);
				...
				if ($this->config['show_subdirs']) {
					foreach( $files as $file ) {
						if( file_exists($root . $dir . $file) && $file != '.' && $file != '..' && is_dir($root . $dir . $file) ) {
							if (!$this->config['show_hidden'] && $this->isHidden($root . $dir, $file)) continue;
	...
	protected function isHidden($path, $name) {
