...
class DataTransform {
	...
	// crmv@48267
	// use this function after reading directly from database
	public static function sanitizeDataReadWithTimezone($row,$meta) {
		...
		$newRow = DataTransform::sanitizeReadTimezoneFields($newRow,$meta);
		$newRow = DataTransform::sanitizeDateFieldsForInsert($newRow,$meta); //crmv@36510
		$newRow = DataTransform::sanitizeFields($newRow,$meta);

		return $newRow;
	}

	// TODO: Tasks and standard date fields (uitype 5, 6)
	public static function sanitizeReadTimezoneFields($row, $meta) {
		global $current_user;

		if ($current_user->timezonediff == 0) return $row;

		if ($meta->getEntityName() == 'Events') {
			if ($row['date_start'] && $row['time_start']) {
				$newval = substr($row['date_start'], 0, 10).' '.$row['time_start'];
				$newval = adjustTimezone($newval, 0, null, false);
				$row['date_start'] = substr($newval, 0, 10);
				$row['time_start'] = substr($newval, 11, 5);
			}
			if ($row['due_date'] && $row['time_end']) {
				$newval = substr($row['due_date'], 0, 10).' '.$row['time_end'];
				$newval = adjustTimezone($newval, 0, null, false);
				$row['due_date'] = substr($newval, 0, 10);
				$row['time_end'] = substr($newval, 11, 5);
			}
		}
	...
	public static function sanitizeForInsert($row,$meta){
		...
		if(strtolower($meta->getEntityName()) == "emails"){
			if(isset($row['parent_id'])){
				...
				$userObj = VtenextWebserviceObject::fromName($adb,'Users');//crmv@207871
		...
		if(strtolower($meta->getEntityName()) == "events"){
			if(isset($row['reminder_time'])&& $row['reminder_time']!= null && $row['reminder_time'] != 0){
				...
				//at vte there cant be 0 minutes reminder so we are setting to 1
				if($minutes == 0){
						$minutes = 1;
				}
	...
	public static function sanitizeFields($row,$meta){
		...
		$recordId = 0; // crmv@205306

		if(isset($row['id'])){
			$recordId = intval($row['id']); // crmv@205306
		...
		// crmv@205306
		if ($recordId > 0 && isset($row['taxclass'])) {
			$instance = vtws_getModuleInstance($meta->getWebserviceObject());
			if (method_exists($instance, 'retrieveTaxes')) {
				$row['taxclass'] = Zend_Json::encode($instance->retrieveTaxes($recordId));
			}
		}
		// crmv@205306e
		...
		foreach($references as $field=>$typeList){
			...
			if($row[$field]){
				...
				foreach ($typeList as $entity) {
					$webserviceObject = VtenextWebserviceObject::fromName($adb,$entity);//crmv@207871
	...
	public static function sanitizeOwnerFields($row,$meta,$t=null){
		...
		foreach($ownerFields as $index=>$field){
			if(isset($row[$field]) && $row[$field]!=null){
				...
				$webserviceObject = VtenextWebserviceObject::fromName($adb,$ownerType);//crmv@207871
				$row[$field] = vtws_getId($webserviceObject->getEntityId(),$row[$field]);
			}
		}
		return $row;
