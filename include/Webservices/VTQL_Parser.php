...
Array ( 
	...
	[tableName] => vte_crmentity,vte_contactdetails,vte_contactaddress,vte_contactsubdetails,vte_contactscf,vte_customerdetails
...
function buildSelectStmt($sqlDump){
	...
	if(in_array('*', $sqlDump['column_list'])){
		...
		$this->query = $this->query." COUNT(*) as 'count(*)'"; //crmv@55311
	}else{
		$i=0;
	...
	if($sqlDump['where_condition']){
		...
		(@sizeof($sqlDump['where_condition']['column_operators']) == @sizeof($sqlDump['where_condition']['operators'])+1)){ // crmv@198899
	...
	$this->query = $meta->getEntitylistQueryNonAdminChange($this->query);	//crmv@38549
	...
	if($sqlDump['limit']){
		...
		if(!empty($sqlDump['limit'])){
			if(count($sqlDump['limit']) == 1){
				$this->limit = array(0,$sqlDump['limit'][0]);
			}
			else{
				$this->limit = array($sqlDump['limit'][0],$sqlDump['limit'][1]);
			}
		}
		...
		*/
		...
		$this->limit = array(0,1000); //crmv@55311 fix for z-push
	}
}
...
// crmv@181748 - undefined constant and useless if
//if(SEMI){
...
$this->out['column_list'][] = 'count(*)';
    ...
    function yy_r7(){
if(!in_array("*", $this->out["column_list"]) && !in_array("count(*)", array_map('strtolower', $this->out["column_list"]))){ // crmv@198899
...
$this->out['where_condition']['column_values'][sizeof($this->out['where_condition']['column_values'])-1] = $value[1];
    ...
    function yy_r17(){
$length = is_array($this->out['where_condition']['column_values']) ? sizeof($this->out['where_condition']['column_values']) : 0; // crmv@181748
$pos = $length - 1;
if($pos < 0){
$pos = 0;
}
...
}elseif( !in_array('count(*)', array_map('strtolower', $this->out['column_list']))){ // crmv@198899
...
if($firstTable!=$table){
	if(!isset($tabNameIndex[$table]) && $table == $table_prefix."_crmentity"){
