...
class Headers implements Countable, Iterator
{
    ...
    // crmv@125817
    // code from the last version of Zend
    /**
    ...
    public static function fromString($string, $EOL = self::EOL)
    {
        ...
        for ($i = 0; $i < $total; $i += 1) {
            ...
            if (preg_match('/^\s*$/', $line)) {
                $emptyLine += 1;
                if ($emptyLine > 2) {
                    //throw new Exception\RuntimeException('Malformed header detected'); // crmv@125817
                }
                continue;
            }
            if ($emptyLine > 0) {
            ...
            if (preg_match('/^[\x21-\x39\x3B-\x7E]+:.*$/', $line)) {
                if ($currentLine) {
					...
					try {
						...
						echo sprintf('Line "%s" does not match header format!',$line)." of this header:\n\n$string\n\n";
					}
					//crmv@65968e
                }
