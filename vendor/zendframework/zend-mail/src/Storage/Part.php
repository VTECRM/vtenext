...
class Part implements RecursiveIterator, Part\PartInterface
{
    ...
    protected function _cacheContent()
    {
        ...
        //crmv@150957
        if (isset($this->boundary) && !empty($this->boundary)) $boundary = $this->boundary;
        else $boundary = $this->getHeaderField('content-type', 'boundary');
        //crmv@150957e
        if (!$boundary) {
    ...
    public function getHeader($name, $format = null)
    {
    	...
    	if (empty($headers)) {
    		throw new Exception\InvalidArgumentException(
            	"Header with Name $name not found"
			);
		}
