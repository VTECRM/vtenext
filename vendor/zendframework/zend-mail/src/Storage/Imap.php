...
class Imap extends AbstractStorage implements Folder\FolderInterface, Writable\WritableInterface
{
    ...
    public $folderSeparator = ''; //crmv@178164
    ...
    public function getRawContent($id, $part = null)
    {
		...
		//crmv@49432
        $content = $this->protocol->fetch('RFC822.TEXT', $id);
        if (empty($content)) {
        	$content = $this->protocol->fetch('BODY[TEXT]', $id);
        }
        return $content;
        //crmv@49432e
    }
    ...
    {
        // crmv@97771
		$number = $this->protocol->search(array('UID '.$id));
		if(!empty($number[0])) return $number[0];
		// crmv@97771e
        ...
        foreach ($folders as $globalName => $data) {
            do {
                if (!$parent || strpos($globalName, $parent) === 0) {
                	if (empty($this->folderSeparator)) $this->folderSeparator = $data['delim']; //crmv@178164
     ...
     * @throws Exception\RuntimeException
     */
    public function createFolder($name, $parentFolder = null, $separator='/')	//crmv@47411
    {
        // TODO: we assume / as the hierarchy delim - need to get that from the folder class!
        if ($parentFolder instanceof Folder) {
            $folder = $parentFolder->getGlobalName() . $separator . $name;
        } elseif ($parentFolder != null) {
            $folder = $parentFolder . $separator . $name;
        } else {
            $folder = $name;
        }
