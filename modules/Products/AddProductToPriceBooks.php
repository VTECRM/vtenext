...
$sql1="select {$table_prefix}_crmentity.crmid, {$table_prefix}_pricebookproductrel.pricebookid,{$table_prefix}_products.unit_price
			...
			inner join {$table_prefix}_products on {$table_prefix}_products.productid={$table_prefix}_pricebookproductrel.productid
...
$prod_price_details = $InventoryUtils->getPriceDetailsForProduct($productid, $pro_unit_price); // crmv@42024
...
$other_text = '
	...
	if($num_rows != $num_prod_rows)
        	$other_text .='<input class="crmbutton small save" type="submit" value="'.$mod_strings['LBL_ADD_PRODUCT_PRICEBOOKS'].'" onclick="return addtopricebook()"/>&nbsp;';
...
$list_header .= '<td class="lvtCol" width="35%">'.getTranslatedString('PriceBook','PriceBooks').'</td>';
...
for($i=0; $i<$num_rows; $i++)
{
	...
	if(! array_key_exists($entity_id, $pbk_array))
	{
		...
		$unit_price = formatUserNumber($prod_cur_price[$pk_currency_id]); // crmv@173281
		...
		$list_body .= '<td><label for="check_'.$entity_id.'">'.$adb->query_result($list_result,$i,"bookname").'</label></td>';
