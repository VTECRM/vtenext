...
$list_query = $LVU->getListQuery("Services");
$list_query .=  " and ".$table_prefix."_service.discontinued<>0 ";
...
$sql1 = "select productid as serviceid from ".$table_prefix."_pricebookproductrel
		 INNER JOIN ".$table_prefix."_crmentity ON ".$table_prefix."_crmentity.crmid = ".$table_prefix."_pricebookproductrel.productid
		 WHERE ".$table_prefix."_crmentity.setype='Services' AND ".$table_prefix."_crmentity.deleted=0 AND pricebookid=?";
...
$prod_price_list = $InventoryUtils->getPricesForProducts($currency_id, $new_prod_array, 'Services'); // crmv@42024
