...
class vt_DateTime {
	...
	function __construct($timearr, $check) {
		if (! isset( $timearr) || count($timearr) == 0 ) {
			...
			$this->setDateTime($timearr['ts']);	//crmv@17001
		} else {
			...
			if(isset($timearr['year']) && $timearr['year'] !== '') { // crmv@186156
	...
	function getThisweekDaysbyIndex($index){
		$week_array = array();
		if($index < 0 || $index > 7){	//crmv@17001
			die("Provided day is invalid");
		}
	...
	function setDateTime($ts) {
		global $calendar_strings;	//crmv@21024
		if (empty($ts)) {
			$ts = time();
		}
		...
		//crmv@21024		//crmv@21024
		$this->dayofweek_inshort =$calendar_strings['cal_weekdays_short'][$this->dayofweek];
		$this->dayofweek_inlong=$calendar_strings['cal_weekdays_long'][$this->dayofweek];
		$this->month_inshort=$calendar_strings['cal_month_short'][$this->month];
		$this->month_inlong=$calendar_strings['cal_month_long'][$this->month];
		//crmv@21024e
		$this->daysinyear = 365;
