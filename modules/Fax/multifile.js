 ...
 */
 function get_ext(path) {
    p_point=path.lastIndexOf(".");
	strlength=path.length;
	ext=path.substring(p_point+1,strlength);
	return ext;
}

function control_ext(path){
 	var fileext= get_ext(path);
	...
	this.addElement = function( element ){
		...
		if( element.tagName == 'INPUT' && element.type == 'file' ){
			...
			element.onchange = function(){
				//control extension of the file
	if (control_ext(this.value)){
			this.value=null;
			alert(alert_arr.no_valid_extension);
			return;
	}
