...
class JobOrder extends CRMEntity {
	var $db, $log; // Used in class functions of CRMEntity

	var $table_name;
	var $table_index= 'joborderid';
	...
	var $list_fields_name = Array(
		...
		'JobOrder Name'=> 'jobordername',
		'Assigned To' => 'assigned_user_id'
	);

	// Make the field link to detail view from list view (Fieldname)
	var $list_link_field = 'jobordername';
	...
	var $popup_fields = Array('jobordername');

	// Placeholder for sort fields - All the fields will be initialized for Sorting through initSortFields
	var $sortby_fields = Array();

	// For Alphabetical search
	var $def_basicsearch_col = 'jobordername';

	// Column value to use on detail view record text display
	var $def_detailview_recname = 'jobordername';

	// Required Information for enabling Import feature
	var $required_fields = Array('jobordername'=>1);

	var $default_order_by = 'jobordername';
	var $default_sort_order='ASC';
	// Used when enabling/disabling the mandatory fields for the module.
	// Refers to vte_field.fieldname values.
	var $mandatory_fields = Array('assigned_user_id', 'createdtime', 'modifiedtime', 'jobordername'); // crmv@177975
	//crmv@10759
	var $search_base_field = 'jobordername';
	//crmv@10759 e

	function __construct() {
		global $log, $table_prefix; // crmv@64542
		parent::__construct(); // crmv@37004
		$this->table_name = $table_prefix.'_joborder';
		$this->customFieldTable = Array($table_prefix.'_jobordercf', 'joborderid');
		$this->entity_table = $table_prefix."_crmentity";
		$this->tab_name = Array($table_prefix.'_crmentity', $table_prefix.'_joborder', $table_prefix.'_jobordercf');
		$this->tab_name_index = Array(
			$table_prefix.'_crmentity' => 'crmid',
			$table_prefix.'_joborder'   => 'joborderid',
			$table_prefix.'_jobordercf' => 'joborderid'
		);
		$this->list_fields = Array(
			/* Format: Field Label => Array(tablename, columnname) */
			// tablename should not have prefix 'vte_'
			'JobOrder Name'=> Array($table_prefix.'_joborder', 'jobordername'),
	...
	function vtlib_handler($modulename, $event_type) {
		...
		if($event_type == 'module.postinstall') {
			...
			if ($moduleInstance->is_mod_light) {	//crmv@106857
				...
				//crmv@92272
				$ProcessesModuleInstance = Vtecrm_Module::getInstance('Processes');
				if ($ProcessesModuleInstance) {
