		...
		function fieldReferenceNames(referenceField){
			...
			function forModule(moduleName){
				...
				return map(function(field){					
					return ['('+name+' : '+'('+moduleName+') '+field['name']+')',label+' : '+'('+modules[moduleName]['label']+') '+field['label']]; //crmv@42329
					},
			...
			vtinst.listTypes(handleError(function(accessibleModules) {
				...
				getDescribeObjects(accessibleModules, moduleName, handleError(function(modules){
					...
					$('#task-emailfields').change(function(){
						...
						input.val((oldvalue ? oldvalue+',' : '')+value);
					});
					var selptype = document.getElementById('task-emailfields');
					//crmv@36510
					fillSelectBox('task-emailfields_sender', modules, moduleName,
										 function(e){return e['type']['name']=='email';});
					$('#task-emailfields_sender-busyicon').hide();
					$('#task-emailfields_sender').show();
					$('#task-emailfields_sender').change(function(){
						var input = $($('#save_sender').get());
						var value = '$'+$(this).val();
						var oldvalue = input.val().trim();
						input.val((oldvalue ? oldvalue+',' : '')+value);
					});
					var selptype_sender = document.getElementById('task-emailfields_sender');
					//crmv@36510 e					
