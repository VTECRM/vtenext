...
/* crmv@155585 - the file has been backported from the latest adodb */
...
class ADODB_mssqlnative extends ADOConnection {
	...
	var $fmtDate = "'Y-m-d'";
	var $fmtTimeStamp = "'Y-m-d H:i:s'"; // crmv@155585 - old format
	...
	var $cachedSchemaFlush = true; // crmv@155585 - disable cache
	var $sequences = false;
	var $mssql_version = '';
	...
	function ServerInfo() {
		...
		$this->SetFetchMode($savem); // crmv@155585
	...
	function CreateSequence($seq='adodbseq',$start=1)
	{
		...
		case 9:
		case 10:
		// crmv@155585 - keep old style sequences
		default:
		...
		if(is_array($this->fields)) {
			foreach($this->fields as $key=>$value) {
				if (is_object($value) && method_exists($value, 'format')) {//is DateTime object
					$this->fields[$key] = $value->format("Y-m-d H:i:s"); // crmv@155585
				}
			}
		}
