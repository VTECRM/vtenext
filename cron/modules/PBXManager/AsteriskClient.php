 ...
 */
function asterisk_IncomingEventCleanup() {
	global $adb, $table_prefix;
	
	$HOURRANGE = 60 * 60;
	$TIMELIMIT = $HOURRANGE * 12; // Delete events older than 'n' hours
	
	$adb->pquery("DELETE FROM ".$table_prefix."_asteriskincomingevents WHERE timer < ? ", array(time() - $TIMELIMIT) );
}
...
function asterisk_handleEvents($asterisk, $adb, $version="1.4") {	
	...
	do {
		...
		if(!empty($mainresponse)) {
			...
			if(asterisk_handleResponse1($mainresponse, $state)) {
...
function asterisk_handleResponse1($mainresponse, $state) {
	global $adb, $table_prefix;
	if(
		...
		$columns = array('uid', 'channel', 'from_number', 'from_name', 'timer', 'flag');
  	    $adb->format_columns($columns);
  	    if (!is_numeric($callerNumber))
  	    	$callerNumber = null;
  	    $params = array($uniqueid, $channel, $callerNumber, $callerName, time(), -1);
    	$sql = "insert into ".$table_prefix."_asteriskincomingevents(". implode(",",$columns) .") values(". generateQuestionMarks($params) .")";
...
function asterisk_handleResponse2($mainresponse, $adb, $asterisk, $state) {
	...
	if(
		...
		$uniqueid = $mainresponse['Uniqueid'];
		$uid_col = 'uid';
		$adb->format_columns($uid_col);
		$adb->pquery("UPDATE ".$table_prefix."_asteriskincomingevents set flag = 1 WHERE $uid_col = ?", array($uniqueid));
	}
	...
	if($parseSuccess) {	
		...
		if(checkExtension($extension)) {
			$uid_col = 'uid';
			$adb->format_columns($uid_col);	
			$sql = "UPDATE ".$table_prefix."_asteriskincomingevents SET to_number=?, callertype=?, timer=?, flag=? WHERE $uid_col=?";
			$adb->pquery($sql, array($extension, $callerType, time(), 0, $uniqueid));
			$callerinfo = $adb->pquery("SELECT from_number,from_name FROM ".$table_prefix."_asteriskincomingevents WHERE $uid_col = ?",array($uniqueid));
			...
			if($adb->num_rows($callerinfo) > 0){
				...
				if(empty($callerNumber) || $callerNumber == '0') {
					...
					$sql = "UPDATE ".$table_prefix."_asteriskincomingevents SET flag=? WHERE $uid_col=?";
					$adb->pquery($sql, array(-1, $uniqueid));
				} else {
					$query = "INSERT INTO ".$table_prefix."_asteriskincomingcalls (refuid, from_number, from_name, to_number, callertype, flag, timer) VALUES(?,?,?,?,?,?,?)";
...
function asterisk_handleResponse3($mainresponse, $adb, $asterisk){
	...
	if($mainresponse['Event'] == 'Link' || ($mainresponse['Event'] == 'Bridge' && $mainresponse['Bridgestate'] == 'Link')){
		...
		$uid_col = 'uid';
		$adb->format_columns($uid_col);
		$checkres = $adb->pquery("SELECT * FROM ".$table_prefix."_asteriskincomingevents WHERE $uid_col=? and (flag = 0 or flag = -1)", array($uid));
		if($adb->num_rows($checkres) > 0) {
			...
			$sql = "UPDATE ".$table_prefix."_asteriskincomingevents SET from_number=?, to_number=?, timer=?, flag=? WHERE $uid_col=?";
			...
			if(checkExtension($extensionCalled)) {
				$columns = array('refuid', 'from_number', 'from_name', 'to_number', 'callertype', 'flag', 'timer');
		  	    $params = array($uid, $callerNumber, $checkresrow['from_name'], $extensionCalled, '', 0, time());
	  	  		$query = "insert into ".$table_prefix."_asteriskincomingcalls(". implode(",",$columns) .") values(". generateQuestionMarks($params) .")";
	...
	// TODO Need to detect the caller number using the Event Information
		$callerNumberInfo = $adb->limitpQuery("SELECT from_number, callertype FROM ".$table_prefix."_asteriskincomingevents WHERE $uid_col=? AND from_number is not NULL",0,1, array($uid));
		...
		$uid_col = 'uid';
		$adb->format_columns($uid_col);
		$eventResult = $adb->pquery("SELECT * FROM ".$table_prefix."_asteriskincomingevents WHERE $uid_col = ? and pbxrecordid is NULL AND flag =0", array($uid));
		...
		if($adb->num_rows($eventResult)){
			...
			if($callerNumber != 'Unknown' && $callerNumber != '0') {				
				...
				$uid_col = 'uid';
				$adb->format_columns($uid_col);					
				$adb->pquery("UPDATE ".$table_prefix."_asteriskincomingevents SET pbxrecordid = ? WHERE $uid_col = ?", array($pbxrecordid, $uid));
				if(!empty($receiver_callerinfo['id'])) {
					$adb->pquery("UPDATE ".$table_prefix."_asteriskincomingevents SET relcrmid = ? WHERE $uid_col = ?", array($receiver_callerinfo['id'], $uid));
				}
			}
			return false;
		}
...
/**
 * Check if extension is configured to user in vte
 */
function checkExtension($ext){
	global $adb, $table_prefix;
	
	$sql = "select 1 from ".$table_prefix."_asteriskextensions where asterisk_extension=?";
