...
class VteCsrf {
	...
	public static function check() {
		...
		return self::$instance->csrf_check();
	}
	...
	public static function get_token() {
		...
		return self::$instance->csrf_get_tokens();
	}
	...
	*/
	...
	public function csrf_get_tokens() {
		...
		if (session_id()) return 'sid:' . $this->csrf_hash(session_id()) . $ip;
	...
	*/
	...
	public function csrf_check_token($token) {
		...
		switch ($type) {
			case 'sid':
				return $value === $this->csrf_hash(session_id(), $time);
			case 'cookie':
	...
	*/
	function csrf_start() {
		if ($GLOBALS['auto-session'] && !session_id()) {
			@session_start();
		}
	}
