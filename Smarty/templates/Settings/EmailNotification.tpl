...
<tbody><tr>
	...
	<div align=center>
			...
			<tr>
				<td width="50" rowspan="2" valign="top"><img src="{'notification.gif'|resourcever}" alt="{$MOD.LBL_MODULE_NAME}" width="48" height="48" border=0 title="{$MOD.LBL_MODULE_NAME}"></td>
...
function fetchSaveNotify(id) {
	...
	jQuery("#editdiv").hide();
	jQuery("#status").show();
	var active = jQuery("#notify_status").val();
	...
	jQuery.ajax({
		url: 'index.php',
		method: 'POST',
		data: 'action=SettingsAjax&module=Settings&file=SaveNotification&active='+active+'&notifysubject='+subject+'&notifybody='+body+'&record='+id,
		success: function(result) {
			if(result.indexOf(":#:FAILURE") > -1) {
				alert(alert_arr.VALID_DATA);
			} else {
				jQuery("#notifycontents").html(result);
			}
			jQuery("#status").hide();
		}
	});
...
function fetchEditNotify(id) {
	...
	jQuery.ajax({
		url: 'index.php',
		method: 'POST',
		data:'action=SettingsAjax&module=Settings&file=EditNotification&record='+id,
		success: function(result) {
			jQuery("#status").show();
