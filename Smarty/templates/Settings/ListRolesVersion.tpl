...
*}
{* crmv@150266 *}
<span class="inactive" style="vertical-align:6px">{if $PENDING_VERSION}{$MOD.LBL_VERSION_PENDING_CHANGES} <a href="javascript:;" onclick="ListRoles.closeVersion()" class="simpleSave">{$APP.LBL_SAVE_LABEL} ({$MOD.VTLIB_LBL_PACKAGE_VERSION|substr:0:1|strtolower}. {$PENDING_VERSION})</a>{else}{$MOD.VTLIB_LBL_PACKAGE_VERSION|substr:0:1|strtolower}. {$CURRENT_VERSION}{/if}</span>&nbsp;
{if $PERM_VERSION_EXPORT}
	<i class="vteicon md-link" title="{$MOD.LBL_EXPORT_VERSION}" onclick="ListRoles.exportVersion()">file_download</i>
{else}
...
<div style="display:none">
	<form enctype="multipart/form-data" name="Import" method="POST" action="index.php">
		...
		<input type="hidden" name="action" value="ListRolesAjax">
