	...
	<table cellspacing="0" cellpadding="0">
		...
		{/if}
			...
			{if $CHART_SHOWDATE && $CHART_LASTUPDATE > 0}
				<span style="text-align:right;font-size:90%;position:absolute;bottom:10px;right:10px"><a style="color: gray; text-decoration: none; " href="javascript:;" title="{$CHART_LASTUPDATE_DISPLAY}">{$MOD.LBL_UPDATED_TO} {$CHART_LASTUPDATE_RELATIVE}</a></span> {* crmv@134727 *}
			{/if}
			{* crmv@128369 *}
			{if $CHART_SHOWREPORTLINK && $REPORTID > 0}
				<span style="text-align:left;font-size:90%;position:absolute;bottom:10px;left:10px"><a href="index.php?module=Reports&amp;action=SaveAndRun&amp;record={$REPORTID}" target="_blank" title="{$MOD.LBL_VIEW_REPORT}">{$MOD.LBL_VIEW_REPORT}</a></span>
			{/if}
			{* crmv@128369e *}
		</td></tr>
