...
<div id="pickListContents">
        ...
        <tr>
            ...
            <td width="30%">
                <div class="dvtCellInfo">
                    <select name="avail_picklists" id="allpick" class="detailedViewTextBox"
            ...
            <td nowrap align="right">
                <input type="button" value="{$APP.LBL_ADD_BUTTON}" name="add" class="crmButton small create"
                       onclick="showAddDiv();">
                <input type="button" value="{$APP.LBL_EDIT_BUTTON}" name="del" class="crmButton small edit"
                       onclick="showEditDiv();">
                <input type="button" value="{$APP.LBL_DELETE_BUTTON}" name="del" class="crmButton small delete"
    ...
    <table class="tableHeading" border="0" cellpadding="0" cellspacing="0" width="100%">
        <tr>
			...
			<select name="pickrole" id="pickid" class="detailedViewTextBox"
                    onChange="showPicklistEntries('{$MODULE}' );" style="width : auto;">
                        ...
                        <tr>
                            {foreach item=picklistfields from=$picklists}
                                {if $picklistfields neq ''}
                                    <td class="listTableTopButtons small" style="padding-left:20px" valign="top"
                                        align="left">
                                    ...
                                    <td class="listTableTopButtons" valign="top">
                                        <input type="button" value="{$MOD_PICKLIST.LBL_ASSIGN_BUTTON}"
                                               class="crmButton small edit"
                                               onclick="assignPicklistValues('{$MODULE}','{$picklistfields.fieldname}','{$picklistfields.fieldlabel}');">
                                    </td>
                                {else}
                                        ...
                                        <ul style="list-style-type: none;">
                                            {foreach item=elements from=$picklistelements.value}
                                                {if $TEMP_MOD[$elements] neq ''}
                                                    <li style="min-height:25px">{$TEMP_MOD[$elements]}</li>
                                                {elseif $MOD_PICKLIST[$elements] neq ''}
                                                    <li style="min-height:25px">{$MOD_PICKLIST[$elements]}</li>
                                                {else}
                                                    <li style="min-height:25px">{$elements}</li>
                                                {/if}
                                            {/foreach}
                                        </ul>
                                    </td>
