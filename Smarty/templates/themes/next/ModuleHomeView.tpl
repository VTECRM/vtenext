	...
	<table id="bl3" border=0 cellspacing=0 cellpadding=2 width=100% class="small">
		<tr height="34">
			<td align="left" valign="middle">
					...
					{foreach item=VIEW from=$MODHOMEVIEWS}
						...
						<td class="{$_class}" align="center" id="tdViewTab_{$VIEW.modhomeid}" nowrap="">
							...
							<div class="dropdown vcenter" id="editModHomeBlocks_{$VIEW.modhomeid}">
								<span id="pencil_{$VIEW.modhomeid}" data-toggle="dropdown" style="width:20px;height:18px;display:none;">
									<i class="vteicon valign-bottom md-sm md-link">settings</i> {* crmv@120023 *}
								</span>
								<ul class="dropdown-menu dropdown-autoclose">
									...
									<li><a href="javascript:void(0);" onclick="ModuleHome.removeView('{$VIEW.modhomeid}', '{$VIEWNAME_SAFE}', true)">{$APP.LBL_REMOVE_MODHOME_VIEW}</a></li>
									<li><a href="javascript:void(0);" onclick="ModuleHome.editView('{$VIEW.modhomeid}', '{$VIEWNAME_SAFE}', '{$VIEW.reportid}', '{$VIEW.cvid}')">{$APP.LBL_EDIT_MODHOME_VIEW}</a></li>{* crmv@199319 *}
									{/if}
				...
				<div class="pull-left" id="add_home_views" style="padding-top:8px;padding-left:8px;display:none">
					<div class="dropdown" id="editModHomeViews">
						<a data-toggle="dropdown">
							<i class="vteicon md-link" style="vertical-align:middle" title="{$APP.LBL_ADD_ITEM} tab">add</i>
