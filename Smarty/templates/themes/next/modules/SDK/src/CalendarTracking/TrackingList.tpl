...
{if count($TRACKLIST) > 0}
	<table class="vtetable" id="track_buttons">
		<tbody>
			{foreach key=id item=info from=$TRACKLIST}
				<tr height="34">
					<td width="140" class="align-middle text-nowrap">
					...
					<td class="align-middle"><a href="index.php?module={$info.module}&action=DetailView&record={$id}" target="_parent">{$info.name}</a></td>
					<td class="align-middle">{$info.entity_type}</td>
				</tr>
			{/foreach}
		</tbody>
	</table>
{else}
	<div class="vte-collection-empty">
		<div class="collection-item">
			...
			<h4 class="title">{$APP.LBL_TRACKING_NO_ENTRIES}</h4>
		</div>
	</div>
{/if}
