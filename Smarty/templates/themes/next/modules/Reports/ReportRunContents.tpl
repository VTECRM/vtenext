...
<table class="table-fixed" cellpadding="0" cellspacing="0" width="100%" width="100%">
	<tbody>
	<tr><td colspan="2">
	...
	<tr><td colspan="2">
		...
		{if $REPORT_HAS_SUMMARY && $DIRECT_OUTPUT eq true}
			<div style="width:70%" class="vte-card"> {* crmv@205568 *}
				{* crmv@181170 *}
		...
		<tr id="trReportMain">
		<td colspan="2" id='table_reports' align="left">
		<div class="container-fluid">
		<div class="row">
		<div class="col-sm-12">
			{* Performance Optimization: Direct result output *}
			{if $DIRECT_OUTPUT eq true}
				<div id="reportMainContainer" class="vte-card">
...
<script type="text/javascript">
	jQuery(document).ready(function() {ldelim}
		...
		// crmv@198701
		var origWidth = parseInt(jQuery('#reportMainContainer').width());
		
		jQuery('#tableContentMain').on('init.dt', function() {ldelim}
			var newWidth = parseInt(jQuery('#reportMainContainer').width());
		
			if (newWidth > origWidth) {ldelim}
				jQuery('body').css('overflow', 'auto');
				jQuery('#mainContent').css('overflow', 'unset');
				
				var rightMenuWidth = parseInt(jQuery('#rightPanel').width());
				jQuery('#reportMainContainer').css('margin-right', rightMenuWidth + 'px');
			{rdelim}
		{rdelim});
		// crmv@198701e
	});
