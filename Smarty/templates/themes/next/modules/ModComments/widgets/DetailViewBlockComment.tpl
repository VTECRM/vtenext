					...
					<table cellpadding="0" cellspacing="0" width="100%">
						...
						{* crmv@43448w *}
							<td align="right">
								...
								<div class="dropdown">
									<button class="crmbutton small save dropdown-toggle" data-toggle="dropdown">
										{$MOD.LBL_PUBLISH}&nbsp;{$MOD.LBL_TO}&nbsp;<span class="caret"></span>
									</button>
									<ul id="ModComments_sub" class="dropdown-menu dropdown-menu-right dropdown-autoclose">
										{if $ENABLE_PUBLIC_TALKS}
											<li><a href="javascript:;" onclick="VTE.ModCommentsCommon.addComment('{$UIKEY}', '{$ID}', 'All', {$INDICATOR});">{'All'|getTranslatedString:'ModComments'}</a></li>
										{/if}
										<li><a href="javascript:;" onclick="showUsersPopup(this)">{'Users'|getTranslatedString:'ModComments'}</a></li>
									</ul>
								</div>
							</td>
						</tr>
...
<div id="ModCommentsUsers" class="crmvDiv" style="display:none;position:fixed;width:350px;">
	...
	<div id="ModCommentsUsers_list" class="txtBox1" style="height:150px;overflow-y:auto;"></div>
	<div class="closebutton" onClick="hideFloatingDiv('ModCommentsUsers'); removeAllUsers();"></div>
</div>
...
<script type="text/javascript">
	...
	function showUsersPopup(self, commentid) {
		...
		showFloatingDiv('ModCommentsUsers', self);
...
{literal}
<script type="text/javascript">
	VTE.ModCommentsCommon.registerPanelBlocker(); // crmv@171115
</script>
{/literal}